...
jQuery("html").on("click", function(e){
	if(jQuery(e.target).closest(".dim-overlay").length){
		alert(1);
...
$(".dropdown-heading").click(function(){
	var n = $(this).next("ul").find("li").length;
	var dropdown = 37*n;
	var _todrop = $(this).next("ul");
	var nodrop = $("#mMenu .has-sub ul");
	if($(this).hasClass("down")){//만약 다운이면
		_todrop.height(0);
	}
	else{
		nodrop.height(0);
		$(".dropdown-heading").removeClass("down");
		_todrop.height(dropdown);
...
function hasScrolled() {
	...
	//=========================================
	if(st>0){//스트롤이 내려졌으면
		$('header').addClass('hide-banner');		
	}
	else{
		$('header').removeClass('hide-banner');
	}
	topMenuPaddingSet(1);
	//=========================================
...
function topMenuPaddingSet(is_height){
	if(is_height==1){
		var height=jQuery("#header header").height();
		jQuery("#topMenuPadding").height(height);
	}
	else{
		jQuery("#topMenuPadding").height(0);
	}
}